...
#         [-0.18507415  0.34132686  0.77874327 -0.62734276 -0.38149858  1.6813459
#    0.9476043  -0.31944653  0.05462402 -0.0126444  -0.15394692  0.98807913
#    0.05242977  0.04313301  0.48442075  0.87483513]

    observation = np.array([-0.18507415,  0.34132686,  0.77874327, -0.62734276, -0.38149858,  1.6813459, 0.9476043,  -0.31944653,  0.05462402, -0.0126444,  -0.15394692,  0.98807913, 0.05242977,  0.04313301,  0.48442075,  0.87483513])
...
def save_as_H5(model, file_path="./utils/exported_model/", test=False):
    verbose = not test
    ...
    action_model = pytorch_to_keras(
        model.policy.action_net, torch.zeros((1, 64)), verbose=verbose, name_policy="renumerate"
    )
    value_model = pytorch_to_keras(
        model.policy.value_net, torch.zeros((1, 64)), verbose=verbose, name_policy="renumerate"
    )
    ...
    with open(file_path + "metadata.json", "w") as metadata_file:
        #Add "policy_type": "MARL" to metadata
        metadata["policy_type"] = "MARL"
    ...
    if test:
        print('Starting Test')
        analyze_keras_model(total_model, action_model, value_model)
        print('Test Complete')


if __name__ == "__main__":
    if not len(sys.argv) == 2 and not len(sys.argv) == 3:
        print("usage: ./export_model.py <model path>")
        exit(1)
